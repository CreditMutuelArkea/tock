...
```bash
gunicorn llm_orchestrator.main:app --reload --bind :8000 --workers=2 --worker-class=uvicorn.workers.UvicornWorker --log-config=~/Code/GitHub/CreditMutuelArkea/tock/llm/orchestrator-server/src/main/python/app/src/llm_orchestrator/configurations/logging/config.ini
```
