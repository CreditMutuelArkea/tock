            ...
            if (user != null && user !is TockUser) { // user is Pac4jUser
                executor.executeBlocking {
                    upgradeToTockUser(getUserCasProfile(rc)) { hr ->
                        if (hr.succeeded()) {
                            (rc.userContext() as UserContextInternal).setUser(hr.result) // TODO : setUser problem (*): This assignment (adding the user to the context) is not maintained for the next vertx handler (rc.next()).
                            rc.session().put("tockUser", hr.result) // TODO : Swe are therefore temporarily using the session to store tockUser.
                            rc.next()
                        } else {
