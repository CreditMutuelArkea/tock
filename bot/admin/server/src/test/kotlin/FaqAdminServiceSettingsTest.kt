...
class FaqAdminServiceSettingsTest : AbstractTest() {
    ...
    private fun initMock(intent: IntentDefinition, faqs: List<FaqDefinition>, stories: List<StoryDefinitionConfiguration>) {
    ...
    fun `GIVEN save faq settings WHEN satisfaction enabled THEN add ending rule to the story`() {
        ...
        assertEquals(slotStory[0].features.count { feature -> feature.endWithStoryId == faqSettings.satisfactionStoryId }, 1)
        assertEquals(slotStory[1].features.size, 1)
        assertEquals(slotStory[1].features.count { feature -> feature.endWithStoryId == faqSettings.satisfactionStoryId }, 1)
    }
