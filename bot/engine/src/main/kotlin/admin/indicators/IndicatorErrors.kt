...
sealed class IndicatorError(override val message: String) : Exception(message) {
    class IndicatorAlreadyExists(val name: String, val label: String, val botId: String) :
        IndicatorError("Indicator already exists for bot $botId with following name: $name or/and label: $label")

    class IndicatorNotFound(val name: String, val botId: String) :
        IndicatorError("Indicator $name not found for bot $botId")

    class IndicatorDeletionFailed(val id: String) :
        IndicatorError("Indicator (_id: $id) deletion failed")
