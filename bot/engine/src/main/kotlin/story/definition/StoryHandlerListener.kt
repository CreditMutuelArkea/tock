...
package ai.tock.bot.story.definition
...
interface StoryHandlerListener {
    ...
    /**
     * Called before [ScriptHandler.handle].
     * If it returns false, the [StoryHandlerListener]s registered after this listener
     * and the [ScriptHandler] are not called.
     * (however [endAction] of each [StoryHandlerListener] is called].
     */
    fun startAction(botBus: BotBus, handler: ScriptHandler): Boolean = true

    /**
     * Called when [ScriptHandler] handling is over.
     */
    fun endAction(botBus: BotBus, handler: ScriptHandler) = Unit
}
