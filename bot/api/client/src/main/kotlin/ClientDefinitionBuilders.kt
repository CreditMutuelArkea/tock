...
/**
 * Creates a new bot [ClientBotDefinition] in bot Api mode with an available transfer story
 * @param apiKey the api key in studio for the bot configuration
 * @param stories List of stories [ClientStoryDefinition] supported by the bot.
 * @param unknownStory the unknown story triggered when unknown intent
 * @param transferStory a story triggered when a transfer to human or another bot is used (not implemented in all connectors)
 */
fun newBot(
    apiKey: String,
    stories: List<ClientStoryDefinition>,
    unknownStory: ClientStoryDefinition = defaultUnknownStory(),
    transferStory: ClientStoryDefinition
): ClientBotDefinition = ClientBotDefinition(apiKey, stories, unknownStory, transferStory)
